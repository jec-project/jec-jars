...
/*!
 * This module constains utilities used by the CONNECTTest test suite.
 */
...
export const KEY:string = "connectMethod";
...
export const initContext:Function = function():JcadContext {
  ...
  const connector = new TestConnector(JarsConnectorRefs.CONNECT_CONNECTOR_REF, TEST_DECORATOR);
  const context:JcadContext = factory.create();
  DecoratorConnectorManager.getInstance().addConnector(connector, context);
  JcadContextManager.getInstance().addContext(JarsConnectorRefs.CONNECT_CONNECTOR_REF, context);
  return context;
}
export const resetContext:Function = function(context:JcadContext):void {
  JcadContextManager.getInstance().removeContext(JarsConnectorRefs.CONNECT_CONNECTOR_REF);
  DecoratorConnectorManager.getInstance().removeConnector(JarsConnectorRefs.CONNECT_CONNECTOR_REF, context);
  context = null;
}
