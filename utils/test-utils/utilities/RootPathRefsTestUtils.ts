...
/*!
 * This module constains utilities used by the RootPathRefsTest test suite.
 */
...
export const initContext:Function = function():JcadContext {
  ...
  const connector = new TestConnector(JarsConnectorRefs.ROOT_PATH_REFS_CONNECTOR_REF, TEST_DECORATOR);
  const context:JcadContext = factory.create();
  DecoratorConnectorManager.getInstance().addConnector(connector, context);
  JcadContextManager.getInstance().addContext(JarsConnectorRefs.ROOT_PATH_REFS_CONNECTOR_REF, context);
  return context;
}
export const resetContext:Function = function(context:JcadContext):void {
  JcadContextManager.getInstance().removeContext(JarsConnectorRefs.ROOT_PATH_REFS_CONNECTOR_REF);
  DecoratorConnectorManager.getInstance().removeConnector(JarsConnectorRefs.ROOT_PATH_REFS_CONNECTOR_REF, context);
  context = null;
}
