...
/*!
 * This module constains utilities used by the GETTest test suite.
 */
...
export const KEY:string = "getMethod";
...
export const initContext:Function = function():JcadContext {
  ...
  const connector = new TestConnector(JarsConnectorRefs.GET_CONNECTOR_REF, TEST_DECORATOR);
  const context:JcadContext = factory.create();
  DecoratorConnectorManager.getInstance().addConnector(connector, context);
  JcadContextManager.getInstance().addContext(JarsConnectorRefs.GET_CONNECTOR_REF, context);
  return context;
}
export const resetContext:Function = function(context:JcadContext):void {
  JcadContextManager.getInstance().removeContext(JarsConnectorRefs.GET_CONNECTOR_REF);
  DecoratorConnectorManager.getInstance().removeConnector(JarsConnectorRefs.GET_CONNECTOR_REF, context);
  context = null;
}
