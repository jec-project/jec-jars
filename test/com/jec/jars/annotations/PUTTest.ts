...
describe("PUT", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.PUT_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.PUT_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.PUT_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.PUT_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@PUT", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(PUTAnnotation, "PUT");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.PUT_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.PUT_CONNECTOR_REF);
    });
    ...
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.PUT_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.PUT_CONNECTOR_REF, context);
    });
