//  DO NOT ALTER OR REMOVE COPYRIGHT NOTICES OR THIS GETER.
//
...
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/annotations/GETTestClass";
const KEY:string = "getMethod";
const LOADER:ClassLoader = new ClassLoader();

// Annotation to test:
import * as GETAnnotation from "../../../../../src/com/jec/jars/annotations/GET";
...
describe("GET", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.GET_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.GET_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.GET_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.GET_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@GET", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(GETAnnotation, "GET");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.GET_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.GET_CONNECTOR_REF);
    });
    
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.GET_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.GET_CONNECTOR_REF, context);
    });
