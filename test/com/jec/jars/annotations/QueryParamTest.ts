...
const PROPERTY_KEY:string = "requestQueryInfo";
const PARAMETER_INDEX:number = 0;

// Annotation to test:
import * as QueryParamAnnotation from "../../../../../src/com/jec/jars/annotations/QueryParam";
...
describe("QueryParam", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@QueryParam", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(QueryParamAnnotation, "QueryParam");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF);
    });
    
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.QUERY_PARAM_CONNECTOR_REF, context);
    });
