...
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/annotations/ExitTestClass";
const LOADER:ClassLoader = new ClassLoader();
const PROPERTY_KEY:string = "exitCall";
const PARAMETER_INDEX:number = 0;

// Annotation to test:
import * as ExitAnnotation from "../../../../../src/com/jec/jars/annotations/Exit";
...
describe("Exit", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.EXIT_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.EXIT_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.EXIT_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.EXIT_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@Exit", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(ExitAnnotation, "Exit");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.EXIT_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.EXIT_CONNECTOR_REF);
    });
    
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.EXIT_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.EXIT_CONNECTOR_REF, context);
    });
