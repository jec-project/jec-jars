...
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/annotations/OPTIONSTestClass";
const KEY:string = "optionsMethod";
const LOADER:ClassLoader = new ClassLoader();

// Annotation to test:
import * as OPTIONSAnnotation from "../../../../../src/com/jec/jars/annotations/OPTIONS";
...
describe("OPTIONS", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.OPTIONS_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.OPTIONS_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.OPTIONS_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.OPTIONS_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@OPTIONS", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(OPTIONSAnnotation, "OPTIONS");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.OPTIONS_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.OPTIONS_CONNECTOR_REF);
    });
    
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.OPTIONS_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.OPTIONS_CONNECTOR_REF, context);
    });
