...
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/annotations/CookieParamTestClass";
const LOADER:ClassLoader = new ClassLoader();
const PROPERTY_KEY:string = "cookieParamInfo";
const PARAMETER_INDEX:number = 0;

// Annotation to test:
import * as CookieParamAnnotation from "../../../../../src/com/jec/jars/annotations/CookieParam";
...
describe("CookieParam", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@CookieParam", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(CookieParamAnnotation, "CookieParam");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF);
    });
    
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.COOKIE_PARAM_CONNECTOR_REF, context);
    });
