...
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/annotations/CONNECTTestClass";
const KEY:string = "connectMethod";
...
describe("CONNECT", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.CONNECT_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.CONNECT_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.CONNECT_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.CONNECT_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@CONNECT", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(CONNECTAnnotation, "CONNECT");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.CONNECT_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.CONNECT_CONNECTOR_REF);
    });
    ...
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.CONNECT_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.CONNECT_CONNECTOR_REF, context);
    });
