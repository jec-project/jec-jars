...
const KEY:string = "postMethod";
const LOADER:ClassLoader = new ClassLoader();

// Annotation to test:
import * as POSTAnnotation from "../../../../../src/com/jec/jars/annotations/POST";
...
describe("POST", ()=> {
  ...
  before(()=> {
    ...
    connector = new TestConnector(JarsConnectorRefs.POST_CONNECTOR_REF, TEST_DECORATOR);
    context = factory.create();
    CTXM.addContext(JarsConnectorRefs.POST_CONNECTOR_REF, context);
    DCM.addConnector(connector, context);
  });

  after(()=> {
    CTXM.removeContext(JarsConnectorRefs.POST_CONNECTOR_REF);
    DCM.removeConnector(JarsConnectorRefs.POST_CONNECTOR_REF, context);
    connector = null;
    context = null;
  });
  ...
  describe("@POST", ()=> {
    ...
    let annotationSpy:any = chai.spy.on(POSTAnnotation, "POST");

    it("should invoke the JcadContextManager with the JarsConnectorRefs.POST_CONNECTOR_REF reference", function() {
      expect(ctxmSpy).to.have.been.called.with(JarsConnectorRefs.POST_CONNECTOR_REF);
    });
    
    it("should invoke the DecoratorConnectorManager with the JarsConnectorRefs.POST_CONNECTOR_REF reference and the correct JCAD context", function() {
      expect(dcmSpy).to.have.been.called.with(JarsConnectorRefs.POST_CONNECTOR_REF, context);
    });
